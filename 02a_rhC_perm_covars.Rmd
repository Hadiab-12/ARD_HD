...
```{r}
set.seed(198)
Permanova_rsca <- mia::runRDA(tse_subset, 
...
saveRDS(Permanova_rsca, "./Permanova_rsca.rds")
# Store results of PERMANOVA test
rda_info <- attr(SingleCellExperiment::reducedDim(Permanova_rsca, "RDA"), "significance")

saveRDS (rda_info, "./rda_info.rds")

# rda_info$permanova |>
  knitr::kable() %>%
  write("./permanova-HTN-full_rhC_covars.csv")

rda_info$homogeneity |>
  knitr::kable() %>%
  write.csv("homogeneity-HTN-full_rhC_covars.csv")
